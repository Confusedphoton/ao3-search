...
| Exploration strategy (beam search) | ε-greedy for expected-info. Topological ranks by fragility. **`topo-query`** adds iterative-deepening best-first A* over node expand/requery and `/works/search` construction (fragility proxy). No maintained top-K frontier beam. |
...
| Stopping conditions | Budget, frontier acquisition threshold, empty frontier, and (topological / topo-query) trivial-Hasse stability. Missing: relevance-distribution convergence check across expansion iterations. (Per-signal propagation still converges internally via tolerance.) |
...
| Maximize info per request | Tag/author listing pagination is implemented; hubs stay `partial` until exhausted. Stale complete hubs can be demoted when AO3 work counts grow. `/works/search` is used by the **`topo-query`** expansion policy; unused by expected-info / plain topological. |
